...
func TestCertVerify(t *testing.T) {
	tests := map[string]struct {
		...
		// The leaf carries no CRL distribution point, so a requested
		// revocation check cannot complete: the result is indeterminate
		// rather than clean, and the command exits nonzero.
		"Verify online without a completed revocation check is indeterminate": {
			enrolled: true, online: true, wantErr: true, wantInOutput: "UNKNOWN",
		},
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			...
			args := []string{"certificate", "verify"}
			if tc.online {
