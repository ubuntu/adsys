...
func prepareEnv(t *testing.T) func() {
	...
	err = os.Setenv("ADSYS_CACHE-DIR", filepath.Join(dir, "cache"))
	require.NoError(t, err, "Setup: can’t set env variable")
	err = os.Setenv("ADSYS_RUN-DIR", filepath.Join(dir, "run"))
	require.NoError(t, err, "Setup: can’t set env variable")
	err = os.Setenv("ADSYS_AD-SERVER", "ldap://adserver")
	require.NoError(t, err, "Setup: can’t set env variable")
	err = os.Setenv("ADSYS_AD-DOMAIN", "adserver.domain")
	require.NoError(t, err, "Setup: can’t set env variable")

	return func() {
		err := os.Unsetenv("ADSYS_SOCKET")
		require.NoError(t, err, "Teardown: can’t restore env variable")
		err = os.Unsetenv("ADSYS_CACHE-DIR")
		require.NoError(t, err, "Teardown: can’t restore env variable")
		err = os.Unsetenv("ADSYS_RUN-DIR")
		require.NoError(t, err, "Teardown: can’t restore env variable")
		err = os.Unsetenv("ADSYS_AD-SERVER")
		require.NoError(t, err, "Teardown: can’t restore env variable")
		err = os.Unsetenv("ADSYS_AD-DOMAIN")
		require.NoError(t, err, "Teardown: can’t restore env variable")
	}
}
