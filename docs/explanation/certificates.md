...
* For new ADSys installations, it's recommended to use **LDAP** enrollment.
* Existing ADSys installations default to **CEPCES** enrollment, which is retained for backwards compatibility.

LDAP enrollment is the preferred method for new deployments because it reduces setup complexity and removes the reliance on third-party dependencies: it runs entirely in-process and requires neither {term}`certmonger` nor the Python-based CEPCES helper on the client, and no certificate enrollment web service roles on the Windows server.

### LDAP enrollment (recommended for new installations)

The **`ldap`** method is a native Go implementation that:

* Discovers Certificate Authorities and certificate templates from {term}`Active Directory` via {term}`LDAP`
* Installs root CA certificates to the system trust store
* Submits signing requests directly to {term}`AD CS` in-process using the {term}`MS-ICPR` protocol (DCOM/RPC) and writes the issued certificate and private key to disk

This method does not require {term}`CEPCES` or Python dependencies on the client, and does not require Certificate Enrollment Web Service (CES) or Certificate Enrollment Policy Web Service (CEP) roles on the Windows server. Because ADSys discovers CA and template data over LDAP *before* it can install the CA certificates, on a freshly joined machine the domain controller's LDAP StartTLS certificate is typically not yet trusted by the client. On this first run ADSys therefore accepts a domain controller certificate signed by a not-yet-installed CA, anchoring trust instead on the mutually authenticated {term}`Kerberos` (GSSAPI) bind and its TLS channel binding — the same directory-level trust that Windows autoenrollment relies on. It still enforces that the certificate matches the domain controller's hostname and is otherwise valid (for example, not expired); once the CA is installed, subsequent refreshes verify the full certificate chain strictly.
