...
func rpcCredentialFromCCache(ccache *krbcredentials.CCache) (sspcredential.CCache, error) {
	if ccache == nil {
		return nil, fmt.Errorf("Kerberos credential cache is nil")
	}

	principal := ccache.GetClientCredentials()
	if principal == nil || principal.UserName() == "" {
		return nil, fmt.Errorf("Kerberos credential cache is missing client principal")
	}
	if len(ccache.Credentials) == 0 {
		return nil, fmt.Errorf("Kerberos credential cache contains no credentials")
	}
