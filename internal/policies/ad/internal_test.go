...
func TestNew(t *testing.T) {
	...
	for name, tc := range tests {
		...
		t.Run(name, func(t *testing.T) {
			...
			adc, err := New(context.Background(), "ldap://UNUSED:1636/", "localdomain", withRunDir(runDir), withCacheDir(cacheDir))
			if tc.wantErr {
...
func TestFetchGPO(t *testing.T) {
	...
	for name, tc := range tests {
		...
		t.Run(name, func(t *testing.T) {
			...
			adc, err := New(context.Background(), "ldap://UNUSED:1636/", "localdomain", withCacheDir(dest))
