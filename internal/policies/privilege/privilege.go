...
`
	...
	for _, entry := range entries {
		...
		if contentSudo != "" {
			sudoSections = append(sudoSections, strings.TrimRight(contentSudo, "\n"))
			headerWritten = true
		}
	}
	if len(sudoSections) > 0 {
		if _, err := sudoersF.WriteString(strings.Join(sudoSections, "\n\n") + "\n"); err != nil {
			return err
		}
	}
