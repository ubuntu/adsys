...
const (
	...
	VarUser = "USER"
	// VarFullUser is the fully-qualified user, e.g. "bob@dom.com".
	VarFullUser = "FULL_USER"
	// VarHostname is the short machine hostname, e.g. "workstation01".
	VarHostname = "HOSTNAME"
	// VarFullHostname is the fully-qualified machine name, e.g. "workstation01.dom.com".
	VarFullHostname = "FULL_HOSTNAME"
...
type Context struct {
	...
	FullUser     string // "" for computer policies
	Hostname     string
	FullHostname string // always built from the machine domain, even in user policies
...
func Expand(value string, ctx Context) (string, error) {
	...
	values := map[string]string{
		VarUser:         ctx.User,
		VarFullUser:     ctx.FullUser,
		VarHostname:     ctx.Hostname,
		VarFullHostname: ctx.FullHostname,
