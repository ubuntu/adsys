...
// upstream in internal/ad); ${USER}, ${FQDN_USER} and ${DOMAIN} are derived from
// it. ${HOSTNAME} and ${FQDN_HOSTNAME} always refer to the local machine, with
// ${FQDN_HOSTNAME} using the machine domain so it stays correct for cross-domain
// logons.
func (m *Manager) dynamicValuesContext(objectName string, isComputer bool) (dynamicvalues.Context, error) {
	...
	fqdnHostname := m.hostname
	if machineDomain != "" && !strings.Contains(m.hostname, ".") {
		fqdnHostname = m.hostname + "." + machineDomain
	}
