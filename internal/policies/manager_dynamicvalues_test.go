...
func TestDynamicValuesContext(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"User policy derives user and domain from the object name": {
			...
			want: dynamicvalues.Context{
				User:         "bob",
				FullUser:     "bob@example.com",
				Hostname:     "workstation01",
				FullHostname: "workstation01.example.com",
				Domain:       "example.com",
			},
		},
		"User from a child domain keeps the user domain but machine full hostname": {
			hostname: "workstation01", objectName: "bob@child.example.com",
			want: dynamicvalues.Context{
				User:         "bob",
				FullUser:     "bob@child.example.com",
				Hostname:     "workstation01",
				FullHostname: "workstation01.example.com",
...
func TestExpandDynamicValues(t *testing.T) {
	...
	userCtx := dynamicvalues.Context{User: "bob", FullUser: "bob@example.com", Domain: "example.com"}
