...
// certSelector validates the mutually exclusive certificate selectors of a
// request. The Cobra client checks them too, but any gRPC client can reach
// these RPCs directly, and the engine resolves "all" first: a caller asking to
// renew or remove one named certificate must never be given every certificate
// instead.
func certSelector(nickname string, all bool) error {
	if all && nickname != "" {
		return status.Error(codes.InvalidArgument, gotext.Get("a nickname cannot be used together with all"))
	}
	if !all && nickname == "" {
		return status.Error(codes.InvalidArgument, gotext.Get("specify a certificate nickname or all"))
	}
	return nil
}
...
func (s *Service) CertRenew(r *adsys.CertItemRequest, stream adsys.Service_CertRenewServer) (err error) {
	...
	if err := certSelector(r.GetNickname(), r.GetAll()); err != nil {
		return err
	}
