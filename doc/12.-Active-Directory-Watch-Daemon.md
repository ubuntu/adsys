...
- watch a list of user-configured directories for changes
- when a change is detected, attempt to locate a `GPT.ini` file at the root of the watched directory, or create one if absent
- if a `GPT.ini` file is found, increment the version stanza of the file by 1, thus triggering a re-download of assets (including scripts) by clients
...
dirs:          # list of directories to watch
  - C:\Windows\SYSVOL\sysvol\testdomain.com     # traditional path
  - \\testdomain.com\SYSVOL\testdomain.com      # UNC path
```
...
C:\path\to\adwatchd.exe -c path\to\config.yml
...
C:\path\to\adwatchd.exe service install -c path\to\config.yml
```
...
1. Stop the `adwatchd` service (via the Services GUI or through the CLI)
1. (Optional) Remove the existing `adwatchd` service from the system (through the CLI)
1. Replace the existing `adwatchd.exe` executable with the new one
1. (Optional) Install the `adwatchd` service (through the CLI)
1. Start the `adwatchd` service (via the Services GUI or through the CLI)
...
For detailed descriptions and configuration options of `adwatchd`, refer to the [Command line reference](11.-Command-line-reference.md) section.
